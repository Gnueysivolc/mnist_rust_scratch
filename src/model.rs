...
impl NeuralNet {
    ...
    pub fn backward(
        ...
        // dL/dW2 = dz2 * a1.T  (outer product via matrix multiply)
        let dz2_col = dz2.clone().insert_axis(ndarray::Axis(1)); // [n, 1]
        let a1_row = a1.insert_axis(ndarray::Axis(0)); // [1, m]
        let dw2 = dz2_col.dot(&a1_row); // [n, m]
        ...
        // dL/dW1 = dz1 * x.T  (outer product via matrix multiply)
        let dz1_col = dz1.clone().insert_axis(ndarray::Axis(1)); // [n, 1]
        let x_row = x.clone().insert_axis(ndarray::Axis(0)); // [1, m]
        let dw1 = dz1_col.dot(&x_row); // [n, m]
