...
use crate::user_test::user_testing_single;
...
fn main() -> Result<(), Box<dyn Error>> {
    let (train_xs, train_ys) = load_mnist_csv(TRAIN_PATH).expect("fail to load mnist train data");
    let (test_xs, test_ys) = load_mnist_csv(TEST_PATH).expect("fail to load mnist test data");
    let xlen = train_xs.first().unwrap().len(); //784 = 28^2
    let ylen = train_ys.first().unwrap().len(); //10
    let mut net = NeuralNet::new(xlen, HIDDEN_SIZE, ylen);

    let start_tme = Instant::now();

    let epochs = EPOCHS;
    const LR: f32 = LEARNING_RATE;
    const CHUNK_SIZE: usize = CHUNK_SIZES;
    ...
    for epoch in 0..epochs {
        ...
        train_xs
            .chunks(CHUNK_SIZE)
            .zip(train_ys.chunks(CHUNK_SIZE))
            .enumerate()
            .for_each(|(chunk_idx, (xs_chunk, ys_chunk))| {
                let mut loss = 0.0;
                xs_chunk
                    .iter()
                    .zip(ys_chunk.iter())
                ...
                println!(
                    "sample {}: loss = {:.4}, train acc = {:.2}%",
                    (chunk_idx + 1) * CHUNK_SIZE,
                    loss / CHUNK_SIZE as f32,
    ...
    println!(
        "total duration: {:.2?}ms",
        start_tme.elapsed().as_secs_f64() * 1e3
    );

    let test_acc = accuracy(&test_preds, &test_labels);
    println!("test acc: {:.2}%", test_acc * 100.0);
    ...
    loop {
        user_testing_single(&test_xs, &test_ys, &net);

        println!("test another? (y/n)");
